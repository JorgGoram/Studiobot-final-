...
import { Save, Loader, Bell, LogOut, Crown } from 'lucide-react';
import { supabase } from '../lib/supabase';
import { motion } from 'framer-motion';
import { Bot } from 'lucide-react';
import { ElegantButton } from './buttons/ButtonEffects';
import { getDaysRemaining, getMinsRemaining } from '../utils';
      ...
      <header className="max-w-7xl mx-auto px-2 sm:px-4 py-2 sm:py-4 flex justify-between items-center">
        ...
        <div className="flex items-center gap-1.5 sm:gap-4">
          {user && userProfile?.plan && (
            <div
              className={`hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-full border ${getPlanBadgeColor(
                userProfile.plan
              )}`}
            >
              <Crown className="w-4 h-4" />
              <span className="text-sm font-medium">{userProfile.plan}</span>
              {getDaysRemaining(userProfile.planEnd) !== null &&
              getMinsRemaining(userProfile.totalUsageMinutes) > 0 ? (
                <span className="text-xs opacity-75">
                  ({getDaysRemaining(userProfile.planEnd)} days left)
                </span>
              ) : (
                'Expired'
              )}
