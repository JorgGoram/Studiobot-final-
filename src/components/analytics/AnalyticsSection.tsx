...
interface AnalyticsSectonPropsType {
  data: {
...
export function AnalyticsSection({ data }: AnalyticsSectonPropsType) {
  const [analytiscData, setAnalyticsData] = useState<{
    total_records: number;
    total_mins: number;
    avg_min: number;
    total_complete: number;
  ...
  const metrics = [
    {
      ...
      title: 'Success Rate',
      value:
        analytiscData.total_records > 0
          ? (
              (analytiscData.total_complete * 100) /
              analytiscData.total_records
            ).toFixed(2) + '%' // Calculate success rate percentage
          : '0',
    ...
    },
    {
      ...
      value: analytiscData.total_complete, // Example: Use total_complete as satisfaction metric
    ...
    },
    {
      ...
      value: analytiscData.avg_min, // Example: Use avg_min as efficiency metric
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {metrics.map((metric, index) => (
          ...
          >
            {/* Background Gradient */}
            <div
              className={`absolute inset-0 bg-gradient-to-br from-${metric.color}-600/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity`}
            ></div>
            ...
            <div className="relative z-10">
              ...
              <div className="flex items-center justify-between mb-6">
                ...
                <span
                  className={`text-${metric.color}-400 text-sm font-medium bg-${metric.color}-400/10 px-2 py-1 rounded-full`}
                >
                  {metric.trend}
                </span>
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <MetricCard
          title="Total Calls"
          value={analytiscData.total_records}
          change={12}
        ...
        <MetricCard
          ...
          value={convertSecondsToHHMMSS(analytiscData.total_mins)}
          change={-3}
          changeLabel="vs. last month"
        ...
        <MetricCard
          title="Active Users"
          value={analytiscData.active_users}
          change={8}
          changeLabel="vs. last month"
          icon={<Users className="w-5 h-5 text-purple-400" />}
                ...
                <div key={index} className="space-y-2">
                  <div className="flex items-center justify-between text-sm mb-2">
                    <div className="flex items-center space-x-2">
                      <div
                        className={`w-2 h-2 rounded-full bg-${goal.color}-400`}
                      ></div>
